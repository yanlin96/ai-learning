...
export default async function IndexPage({ searchParams }: { searchParams: Promise<{ url?: string }> }) {
  ...
  return <main className="!pl-0"><SiteHeader variant="home"/>
    <section className="mx-auto grid w-full max-w-[1280px] gap-5 px-5 pt-5 pb-12 sm:px-8">
      <div className="relative isolate overflow-hidden rounded-[28px] bg-[#071329] px-6 py-8 text-white sm:px-10 sm:py-10">
        ...
        <div className="flex flex-wrap gap-3"><a href="/website-audit" className="inline-flex min-h-12 items-center gap-3 rounded-xl bg-cyan-200 px-5 py-3 text-sm font-extrabold text-[#071329] no-underline transition hover:bg-cyan-100 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan-200">Start a website audit <ArrowUpRight size={18}/></a><a href="/smoke-test" className="inline-flex min-h-12 items-center gap-3 rounded-xl border border-white/20 bg-white/5 px-5 py-3 text-sm font-bold text-white no-underline transition hover:bg-white/10">Check a release <Workflow size={17}/></a></div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1.35fr_1fr]">
        <a href="/website-audit" className="group relative overflow-hidden rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-white p-6 text-[#090d46] no-underline transition hover:border-blue-400 hover:shadow-lg focus-visible:outline-2 focus-visible:outline-blue-600">
        ...
        <div className="grid gap-4">
          <a href="/smoke-test" className="rounded-2xl border border-slate-200 bg-white p-5 no-underline transition hover:border-teal-300 hover:shadow-md"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-teal-50 text-teal-700"><Workflow size={21}/></span><h3 className="m-0 flex-1 text-lg font-bold text-[#090d46]">Smoke testing</h3><ArrowUpRight size={18} className="text-slate-400"/></div><p className="mb-0 text-sm leading-6 text-slate-600">Check up to 100 priority and sitemap pages. Review every checked URL before a release.</p></a>
          <div className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="m-0 flex items-center gap-2 text-base font-bold text-[#090d46]"><History size={18} className="text-[#00539d]"/> Pick up where you left off</h3><div className="mt-3 grid gap-2 sm:grid-cols-2"><a href="/history" className="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-3 py-3 text-sm font-bold text-[#00539d] no-underline hover:bg-blue-50">Audit history <ArrowUpRight size={15}/></a><a href="/smoke-test/history" className="flex items-center justify-between gap-2 rounded-lg bg-slate-50 px-3 py-3 text-sm font-bold text-[#00539d] no-underline hover:bg-blue-50">Smoke history <ArrowUpRight size={15}/></a></div><p className="mb-0 text-xs leading-5 text-slate-500">History is stored in this browser, not shared between accounts. Audit history lasts for this tab.</p></div>
        </div>
      </div>
      <div className="grid gap-4 lg:grid-cols-[1.35fr_1fr]">
        ...
        <a href="/disruptions" className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 no-underline transition hover:border-blue-300"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-white text-[#00539d]"><TrainFront size={22}/></span><span className="flex-1"><strong className="block text-base text-[#090d46]">Your commute, covered.</strong><span className="mt-1 block text-sm text-slate-600">Search live notices across all Metro lines.</span></span><ArrowUpRight size={19} className="shrink-0 text-slate-400"/></a>
      </div>
    </section><SiteFooter/>
  </main>;
}
