...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; callbackUrl?: string; signedOut?: string }> }) {
  ...
  return <main className="!pl-0 grid min-h-screen place-items-center bg-[#071329] px-5 text-white">
    <section className="w-full max-w-sm rounded-2xl bg-white p-7 text-[#090d46] shadow-xl">
      <h1 className="m-0 text-2xl font-extrabold">{params.error ? "Sign-in did not finish" : signedOut ? "You’re signed out" : "Connecting to Okta"}</h1>
      {signedOut && !params.error && <p role="status" className="mt-4 text-sm leading-6 text-slate-600">You’ve signed out of CPA Tools. Your Okta session may still be active. Continue when you’re ready to sign in again.</p>}
      ...
      {isOktaConfigured ? <WorkspaceSignInButton callbackUrl={callback} autoStart={!params.error && !signedOut}/> : <p role="status" className="mt-5 text-sm leading-6 text-amber-900">Okta setup is not complete. Configure authentication environment variables and restart or redeploy. Workspace access remains locked.</p>}
