...
- `app/(workspace)/page.tsx`: authenticated sidebar-free workspace launchpad at `/`; old `/?url=...` links redirect to `/website-audit?url=...`. The shared shell removes the desktop offset on home, hides its sidebar and omits the mobile toolbox button.
- `app/(workspace)/layout.tsx`: server-side session boundary for every workspace page, mounting one persistent `app/workspace-shell.tsx` with the shared header outside individual page content. Route groups do not change public URLs. The shell uses the current pathname to reserve feature-page sidebar space; page mains override legacy global padding. Internal navigation uses Next.js Link so account/daily-brief state and desktop group choices survive route changes. External links remain ordinary anchors. API and middleware authentication guards remain authoritative on every request; the persistent layout is not a replacement for them.
