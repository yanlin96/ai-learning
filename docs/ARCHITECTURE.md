...
- `app/workspace-assistant.tsx` and `lib/workspace-assistant.ts`: signed-in, shell-persistent command surface with a deterministic intent fast path. It recognizes a bounded English/Chinese tool vocabulary plus tested common misspellings and extracts only user-supplied HTTP(S) URLs. Unmatched natural language is sent to authenticated `/api/assistant/interpret`, where `lib/workspace-assistant-ai.ts` uses OpenAI Structured Outputs to select one fixed intent enum. The model cannot supply a URL or arbitrary action; application code maps the enum and locally extracted URL back to the allow-list. If the key or model is unavailable, the endpoint returns the safe generic fallback. Supported checks call only their existing protected APIs; normal authentication, public-network validation and report construction remain authoritative. Completed reports are saved under a random ID in tab-scoped `sessionStorage`; the still-open conversation presents a Next.js link to authenticated `app/(workspace)/reports/[id]/page.tsx`, and does not navigate until the user clicks. `app/assistant-report-page.tsx` reuses the normal result components in report-only mode. These URLs are reloadable in the creating tab but are not durable or shareable across browsers/devices; missing local state produces an explicit unavailable state.
...
- `lib/accessibility-audit.ts` and `lib/accessibility-estimate.ts`: a dedicated, bounded accessibility workflow for accessible names, text alternatives, labels, headings, language and frame titles. `lib/accessibility-page-state.ts` conservatively detects access-denied/challenge documents and selects rendered or initial markup: browser-only blocking produces an explicit partial result, while blocking on both paths produces no report. `lib/audit-scoring.ts` converts only genuine page findings into an estimate; confidence falls when browser rendering is unavailable or blocked and the result never represents WCAG compliance.
